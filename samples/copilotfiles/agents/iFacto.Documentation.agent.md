...
- Run `al_symbolsearch` to enumerate existing objects
- Run `al_getdiagnostics` for build health
...
- **waldo** (`waldo-company`) — iFacto documentation conventions and naming
...
When JIRA references are detected, use the `jira-enrichment` skill to retrieve issue details for changelog entries and documentation context.
